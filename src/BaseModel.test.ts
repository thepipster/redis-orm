...
describe('Model:Redis:BaseModel', () => {
    ...
    beforeAll((done) => {
        ...
        Settings.redis.once("connect", async (err) => {
    ...
    test('versioning: __v is incrementing', async (done) => {
        
        let tmp1 = new TestModel(testInfo);

        expect(tmp1.__v).toEqual(1);

        // Should get bumped after a save
        await tmp1.save();
        expect(tmp1.__v).toEqual(2);

        await tmp1.save();
        expect(tmp1.__v).toEqual(3);

        // Should persist after a reload
        let tmp2 = await TestModel.loadFromId(tmp1.id)
        expect(tmp2.__v).toEqual(3);

        done()

    })

    test('versioning: race condition', async (done) => {
        ...
        testState2.aNumber = 12345 // <!-- should keep this
